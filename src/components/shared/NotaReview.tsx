...
export default function NotaReview(props: NotaReviewProps) {
  function notaParaEstrelas(nota: number) {
    ...
    for (let i = 1; i <= 5; i++) {
      if (nota >= i) {
        estrelas.push(<IconStarFilled size={props.tamanho ?? 12} key={i} />);
      } else if (nota >= i - 0.5) {
        estrelas.push(
          <IconStarHalfFilled size={props.tamanho ?? 12} key={i} />
        );
      } else {
        estrelas.push(<IconStar size={props.tamanho ?? 12} key={12} />);
      }
    }
    return estrelas;
  }
